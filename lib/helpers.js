...
function readActionArguments(
  ...
  const method = config.methods.find((m) => m.name === action.method.name);
  ...
  method.params.forEach((paramDefinition) => {
    const value = paramValues[paramDefinition.name];
    if (_.isNil(value)) {
    ...
    paramValues[paramDefinition.name] = parsers.resolveParser(parserToUse)(tryParseJson(value));
  });
